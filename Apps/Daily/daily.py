...
def DailyQ(usrQ, sid):
    ...
    aimlPath = path + "/temp"  # 存放临时文件和aiml文件的路径

    # _______check if learning_______
    isLearn = False
    ckLearn = open(aimlPath + "/learn.txt", "r", encoding='utf-8')
    ...
    if (isLearn):
        ckLearn = open(aimlPath + "/learn.txt", "w", encoding='utf-8')
        ckLearn.write(f_data)
        ckLearn.close()
        # ________将记录文件转换为aiml________
        LearnLog = open(aimlPath + "/learn.txt", "r", encoding='utf-8')
        LearnAiml = open(aimlPath + "/learn.aiml", "r", encoding='utf-8')
        ...
        LearnAiml = open(aimlPath + "/learnNew.aiml", "w", encoding='utf-8')
        LearnAiml.write(aiml_data)
        LearnAiml.close()

        # ________转码________
        LearnAiml = open(aimlPath + "/learnNew.aiml", "r", encoding='utf-8')
    ...
    ans = res.replace("ans=", '')
    if (ans.find("#learn") > -1):
        LearnLog = open(aimlPath + "/learn.txt", "a+", encoding='utf-8')
